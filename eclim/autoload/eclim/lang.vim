...
function! eclim#lang#Search(command, singleResultAction, argline)
  ...
  if !empty(results)
    ...
    elseif len(results) == 1 && a:singleResultAction != "lopen"
      ...
      "modified by showgood since I want to see the line matching the patterns
      "as well, not just location list
      let entries = []
      for result in results
          let entry = {}
          let entry.filename = result.filename
          let entry.lnum = result.line
          let searchedFor = substitute(argline, '.*-p \(.\{-}\)\( .*\|$\)', '\1', '')
          let entry.col = result.column
          let entry.text = readfile(result.filename)[entry.lnum - 1]
          call add(entries, entry)
      endfor
      call eclim#util#SetQuickfixList(entries, 'r')
      exec 'botright copen'
    endif
    return 1
  else
